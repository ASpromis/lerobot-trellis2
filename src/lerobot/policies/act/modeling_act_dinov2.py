...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.env_state_feature:
            n_1d_tokens += 1
        if self.config.image_features:
            n_1d_tokens += len(self.config.image_features)  # for each camera
        self.encoder_1d_feature_pos_embed = nn.Embedding(n_1d_tokens, config.dim_model)
        # Note: No longer need 2D positional embeddings since we use pooled DINOv2 features
        # if self.config.image_features:
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        encoder_in_pos_embed = [self.encoder_1d_feature_pos_embed.weight[0].unsqueeze(0).expand(batch_size, -1)]
        # Robot state token.
        if self.config.robot_state_feature:
            encoder_in_tokens.append(self.encoder_robot_state_input_proj(batch["observation.state"]))
            encoder_in_pos_embed.append(self.encoder_1d_feature_pos_embed.weight[1].unsqueeze(0).expand(batch_size, -1))
        # Environment state token.
        if self.config.env_state_feature:
            encoder_in_tokens.append(
                self.encoder_env_state_input_proj(batch["observation.environment_state"])
            )
            # Get the appropriate position embedding index
            pos_idx = 1 + (1 if self.config.robot_state_feature else 0)
